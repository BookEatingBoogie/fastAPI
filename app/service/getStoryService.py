...
story_system_prompt = f"""You are an assistant that writes fairy tales for young children between the ages of 7 and 9. Your stories should be easy to understand, emotionally warm, and imaginative. Always write in a friendly, age-appropriate tone. Do not use complex vocabulary or abstract ideas.
  **Important:**
  - When generating a new “intro,” vary pacing, tone, sentence structures, and imagery to keep each opening fresh and different.  
  - When continuing an existing story beyond the intro, preserve the same core message or theme while you vary wording and scene details.
  - Use random seeds for lexical/style variation, but always fully describe each key action before moving on.
  - Never skip or compress actions.
  - Resolve the one main event by the final scene (small side-stories allowed if they support the main plot).
  ...
  **Vocabulary Variation**
  - Avoid using the same nouns or verbs in both “intro” and “question” whenever possible.
  - Keep overall repetition below 10% of the vocabulary.
...
def generateFinalQuestion(choice, charName, responseId): # select: 질문 선택지, charName: 주인공 이름
  ...
  # 요청 프롬프트
  storyPrompt = f"""Continue the story of with "{choice}" chosen by {charName} in the previous scene.
  ...
  2. "question" (Korean):
  - Write a narrative-style question that directly asks about the pivotal decision prompted by the final sentence.
  - Use short, clear sentences; you may split into two sentences if needed.
  - Focus only on the choice or action presented in the last sentence; do not reference its surroundings or earlier context.
  - You may use emotional vocabulary to reflect the character’s mood or atmosphere.
  - Only when asking an identification or emotion-based question should you request the answer as a concrete visual noun.

  3. "options" (Korean):
  - For decision questions, provide exactly three verbs in “-기”/“-하기” form.
  - For identification or emotion questions, provide exactly three single nouns (no modifiers).
  - All options must be concrete and visually specific enough for a sticker illustration.
  - Only abstract nouns related to emotions are allowed when relevant; they must use simple vocabulary.
  - Avoid template-like repetition; select fresh, appropriate words each time.
...
def generateEnding(choice, charName, responseId):
  ...
  Avoid rushed conclusions or vague endings. Show what the main character experiences or learns through the final event and across full story.
  Write the ending in Korean, about 400 characters. Do not include any extra explanations or formatting."""
